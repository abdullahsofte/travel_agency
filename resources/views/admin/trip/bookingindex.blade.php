                ...
                <div class="card my-2">
                    ...
                    <div class="card-body table-card-body">
                        <form method="post" action="{{ (@$tripData) ? route('trip.update', $tripData->id) : route('trip.store') }}" enctype="multipart/form-data">
                            ...
                            @php
                                $point = App\Models\Customer::find(@$booking->customer_id);
                                $agentPoint = @$point->agent_point;
                            @endphp
                            ...
                            <div class="row">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="airbus_id" id="airbus_id" class="form-control form-control-sm shadow-none">
                                                ...
                                                @if (isset($tripData->airbus))
                                                    @foreach ($airbus as $item)
                                                        <option value="{{ $item->id }}" {{ $item->id == $tripData->airbus_id ? 'selected' : '' }}>
                                                            {{ $item->name }}
                                                        </option>
                                                    @endforeach
                                                @endif
                                        ...
                                        <div class="col-sm-3">
                                            <input type="date" name="start_date" value="{{ @$tripData->start_date ? @$tripData->start_date : date('Y-m-d') }}" class="form-control form-control-sm shadow-none">
                                        ...
                                        <div class="col-sm-3">
                                            <input type="number" name="price" id="price" value="{{ (@$tripData) ? @$tripData->price : old('price') }}"  class="form-control form-control-sm shadow-none price">
                                        ...
                                        <div class="col-sm-3">
                                            <input type="number" name="agent_point_amount" id="price_per"  value="{{ (@$point) ? @$point->agent_point : old('agent_point') }}" class="form-control form-control-sm shadow-none per_price">
                                            @error('agent_point_amount') <span style="color: red">{{$message}}</span> @enderror
                                        </div>
                                        <label for="agent_point_amount" class="col-sm-2 col-form-label">Point</label>
                                        <div class="col-sm-4">
                                            <input type="number" name="total_point" id="points" value="{{ (@$tripData) ? @$tripData->total_point : old('total_point') }}" class="form-control form-control-sm shadow-none points" readonly>
                                            @error('total_point') <span style="color: red">{{$message}}</span> @enderror
                                        </div>
                                        @endif
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group row">
                                        ...
                                        <label for="title" class="col-sm-3 col-form-label">Payment Type <span style="color: red;">*</span></label>
                                        <div class="col-sm-9">
                                            <select name="payment_type" class="form-control form-control-sm shadow-none">
...
<script>
    $(document).ready(function(){
        $("#price").on("keyup", function(){
            var price = $('.price').val();
            var per_point = $('.per_price').val(); 
            var points =parseFloat(price) / parseFloat(per_point);
    
            $(".points").val(points.toFixed(2));
        });
  
        $("#price_per").on("keyup", function(){
            var price = $('.price').val();
            var per_point = $('.per_price').val(); 
            var points =parseFloat(price) / parseFloat(per_point);
